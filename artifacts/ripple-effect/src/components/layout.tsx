...
import { LogOut, Menu, X, Globe, Library, LineChart, BarChart2, Zap } from 'lucide-react';
          ...
          <div className="flex items-center gap-6">
            ...
            <nav className="hidden md:flex items-center gap-1 text-sm font-medium text-muted-foreground ml-4">
              <Show when="signed-in">
                ...
                <Link href="/products/ripple-lab" className="px-3 py-2 rounded-md hover:text-foreground hover:bg-accent transition-colors flex items-center gap-1.5">
                  <Zap size={14} />
                  {t('nav.rl')}
                </Link>
              </Show>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-16 z-40 bg-background border-b border-border p-4 flex flex-col gap-4 animate-in slide-in-from-top-2">
          <Show when="signed-in">
            ...
            <Link href="/products/ripple-lab" className="flex items-center gap-2 px-4 py-3 rounded-md hover:bg-accent text-lg font-medium">
              <Zap size={20} />
              {t('nav.rl')}
            </Link>
          </Show>
