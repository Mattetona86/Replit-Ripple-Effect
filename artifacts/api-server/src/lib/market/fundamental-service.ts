...
export async function getFundamentalAnalysis(
  ...
  const [
    ...
    getIncomeStatements(symbol, "annual", 7).catch(() => []),
    getIncomeStatements(symbol, "quarter", 8).catch(() => []),
    getBalanceSheets(symbol, "annual", 7).catch(() => []),
    getBalanceSheets(symbol, "quarter", 8).catch(() => []),
    getCashFlows(symbol, "annual", 7).catch(() => []),
    getCashFlows(symbol, "quarter", 8).catch(() => []),
    getKeyMetrics(symbol, "annual", 7).catch(() => []),
    getKeyMetrics(symbol, "ttm", 1).catch(() => []),
    getFinancialRatios(symbol, "annual", 6).catch(() => []),
